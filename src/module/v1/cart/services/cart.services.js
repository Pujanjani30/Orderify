...
const cartAdd = async (data) => {
  ...
  let count = await Cart.countDocuments({ cart_fk_user_id: data.id });

  // if count is 0 then create new cart
  if (count == 0)

    // if cart item qty is 0 then throw error
    if (data.cart_items.some(item => item.cartitm_prd_qty == 0))
      throw new Error('INVALID_DATA');
    else
      return await Cart.create({ cart_fk_user_id: data.id, cart_items: data.cart_items });

  // if count is not 0 then find cart based on user id
  const cart = await Cart.findOne({ cart_fk_user_id: data.id });
...
const cartDelete = async (data) => {
  ...
  return await Cart.findOneAndUpdate({ cart_fk_user_id: data.id },
